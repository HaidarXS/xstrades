<?php
return [
    'MARKETS' => '交易市场',
    'SHARES' => '股票',
    'INDICES' => '指数交易',
    'METALS' => '贵金属交易',
    'ENERGIES' => ' 能源',
    'TRADING' => '交易',
    'COMMODITIES' => ' 大宗商品',
    'CRYPTOS' => '加密货币交易',
    "FOREX" => "货币对",
    'JOIN_XS' => '立即加入XS',
    'MESSAGE_US' => '联系我们',
    'HELP_CENTER' => '帮助中心',

    //markets
    'TRADE_GLOBAL_MULTI_ASSET'=>'携手最佳多资产经纪商进入全球金融市场交易',
    'TRADE_GLOBAL_MULTI_ASSET_DISC'=>'通过 XS 探索在线交易世界，您可以在全球最大的金融市场进行交易，并通过一个账户访问 1000 多种交易工具和多种资产类别。 <br/> <br/>
    交易多种资产类别，包括外汇、股票、金属、期货、商品和指数。 体验低点差、有竞争力的定价、无缝执行，并受益于我们专业的专家和市场分析师团队提供的高质量市场研究。',
    'WHY_TRADE_WITH_XS'=>'为何与 XS 进行交易？',
    'WHY_TRADE_CDF'=>'为什么选择XS进行差价合约交易？',
    'WHY_TRADE_CDF_DISC'=>'了解使用 XS 交易差价合约的优势。 享受优化的交易体验。',
    'LEVERAGE_AND_FLEXIBILITY'=>'杠杆和灵活性',
    'LEVERAGE_AND_FLEXIBILITY_DISC'=>'通过 XS 最大限度地发挥您的交易潜力。 受益于杠杆，让您能够以较小的投资扩大市场敞口。 享受更高的交易策略灵活性。',
    'LONG_AND_SHORT'=>'多头和空头交易',
    'LONG_AND_SHORT_DISC'=>'超越传统的买卖。 XS 使您能够从上涨和下跌的市场中受益。 通过做多来利用看涨趋势，或者通过做空来利用看跌趋势。',
    'FRACTIONAL_TRADING'=>'零星交易',
    'FRACTIONAL_TRADING_DISC'=>'无限制地多元化您的投资组合。 XS 提供分数交易，使您能够以较小的预算投资多种资产类别。 扩大您的市场参与度并增强投资组合多元化。',

    //Metals page
    'METALS_DESC' => '通过XS探索金属交易的巨大可能性，您可以与屡获殊荣的经纪商交易黄金、白银、铂金、鈀金和其他金属的差价合约。 XS 提供了一个探索现货金属交易的无缝平台，让您能够探索新的交易机会。',
    'UNLOCK_THE_POTENTIAL' => '通过 XS 释放金属交易的潜力',
    'UNLOCK_THE_POTENTIAL_DESC' => 'Unleash the Power of Trading the worlds leading stock indices, and Trade International Stock Indices with XS. Gain Access to Global Markets and Seamlessly Buy and Sell Leading stock indices such as Dow Jones, Dax, Nasdaq, and More <br/>Join XS.com today and and unleash the power of stock indices trading',
    'METALS_TRADING_WITH_XS' => '通过 XS 进行金属交易',
    'METALS_TRADING_WITH_XS_DESC' => '无论您选择交易黄金差价合约、白银差价合约、铂金差价合约还是钯金差价合约，XS 都可以为您提供贵金属和贱金属的交易机会，使您能够实现投资组合多元化并对冲通胀。
    <br/>
    受益于金属与其他资产类别的低相关性或负相关性，降低波动性和风险。',
    'UNDERSTADING_METALS' => '了解金属交易：XS 概述',
    'UNDERSTADING_METALS_DESC_1' => '金属交易涉及各种贵金属和贱金属的买卖，为投资者提供了接触这一特定资产类别的机会。 以下是使用 XS 进行金属交易的概述：',
    'UNDERSTADING_METALS_DESC_2' => '多种金属：XS 提供多种金属选择，包括金、银、铂和钯等贵金属，以及铜和铝等贱金属。 每种金属都有其独特的特性和市场动态。',
    'UNDERSTADING_METALS_DESC_3' => '避险资产：黄金和白银等贵金属通常被认为是避险资产，因为它们在市场不确定时期往往会保持其价值。 交易者经常利用这些金属来对冲通胀和经济波动。',
    'UNDERSTADING_METALS_DESC_4' => '工业应用：铜和铝等贱金属对于建筑、电子和制造等各个行业至关重要。 交易这些金属可以让交易者接触到全球工业部门，并允许交易者利用市场趋势和需求波动。',
    'UNDERSTADING_METALS_DESC_5' => '供需动态：金属价格受到供需因素的影响，包括生产水平、地缘政治事件、经济指标和市场情绪。 了解这些动态对于金属交易中明智的决策至关重要。',
    'MORE_INFO' => '欲了解更多信息，请访问我们的帮助中心',
    'TRADE_WITH_THE_BEST_MULTI_ASSET_BROKER' => 'DIVERSIFY YOUR PORTFOLIO WITH THE BEST MULTI-ASSET BROKER',

    //Energy Page
    'ENERGIES_DESC_1' => 'XS Group is a global fintech and financial services provider with entities regulated and authorised by leading governing bodies of the most reputable financial jurisdictions around the globe.',
    'ENERGIES_DESC_2' =>'We prioritise regulatory compliance to ensure trust, integrity, and accountability in the global financial industry. Our operations adhere to the regulations and requirements set forth by the relevant authorities in each jurisdiction where we operate.',
    'ENERGIZE_YOUR_TRADING' => '与全球经纪商一起激发您的交易并探索能源市场',
    'ENERGIZE_YOUR_TRADING_DESC' => '当您探索令人兴奋的能源市场时，通过 XS 为您的交易之旅提供动力。 交易多种能源商品，包括原油、天然气等，并抓住全球供需动态、地缘政治事件和技术进步驱动的机遇。',

    'ENERGY_TRADING_WITH_XS' => '使用 XS 进行能源交易',

    'ENERGY_TRADING_WITH_XS_DESC' => '通过XS，您可以享受我们优势的交易环境，具有具有竞争力的价格、低廉的点差和高效的执行。 利用我们最先进的 MetaTrader 4 和 MetaTrader 5 交易平台，可跨多种设备访问，确保无缝交易体验。',

    'UNDERSTANDING_ENERGIES' => '了解加密货币交易：XS概述',

    'UNDERSTANDING_ENERGIES_DESC_1' => '能源商品交易提供了参与全球能源行业，利用供求动态、地缘政治事件和市场趋势带来的价格波动的一个独特的机会。在本概述中，我们将探讨能源交易的关键点及其潜在好处。',

    'UNDERSTANDING_ENERGIES_DESC_2' => '能源交易简介 <br/> 能源交易涉及原油、天然气、汽油、取暖油等能源商品的买卖。这些商品是为工业、交通和日常生活提供动力的重要资源。能源交易为投资者和交易商提供了一个平台，让他们可以对这些商品的价格走势进行投机，并从中获取潜在利润。',

    'UNDERSTANDING_ENERGIES_DESC_3' => '推动能源价格的因素<br/> 能源价格受到一系列因素的影响。 供需动态发挥着至关重要的作用，地缘政治事件、天气状况和全球经济趋势影响着供需平衡。 政府政策、技术的进步和环境法规也会影响能源价格。 随时了解这些因素是做出明智交易决策的关键。',

    'UNDERSTANDING_ENERGIES_DESC_4' => '市场机会和波动性<br/> 能源市场以其波动性著称，为交易者从价格波动中获利创造了机会。全球事件和供需变化所推动的市场趋势会导致价格大幅波动。交易者可以利用能源价格的上涨和下跌趋势，在看涨和看跌的市场条件下都能获得潜在利润。',

    'UNDERSTANDING_ENERGIES_DESC_5' => '通过 XS 进入能源市场<br/> XS提供全面的交易平台，提供进入能源市场和各种能源商品的机会。 凭借实时市场数据、先进的交易工具和有竞争力的定价，XS 使交易者能够做出明智的决策并高效地执行交易。 我们的平台确保可靠的执行，并支持交易者在有效管理风险的同时最大化潜在回报。',

    //Crypto Page
    'CRYPTOS_DESC_1' => 'XS Group is a global fintech and financial services provider with entities regulated and authorised by leading governing bodies of the most reputable financial jurisdictions around the globe.',
    'CRYPTOS_DESC_2' => 'We prioritise regulatory compliance to ensure trust, integrity, and accountability in the global financial industry. Our operations adhere to the regulations and requirements set forth by the relevant authorities in each jurisdiction where we operate.',
    'JOIN_THE_DIGITAL_REVOLUTION' => '通过加密货币交易加入数字革命',
    'JOIN_THE_DIGITAL_REVOLUTION_DESC' => '通过 XS 探索加密货币世界并释放数字资产的潜力。 交易比特币、以太坊、莱特币等流行的加密货币，并在快速发展的加密货币市场中抓住机遇。 立即加入 XS.com，开始您的加密货币交易之旅。',
    'CRYPTOCURRENCY_TRADING_WITH_XS' => '使用 XS 进行加密货币交易',
    'CRYPTOCURRENCY_TRADING_WITH_XS_DESC' => '通过 XS.com 体验加密货币交易的刺激。 我们的平台提供多种加密货币的无缝交易，让您能够利用市场波动和潜在的价格变动。 受益于我们先进的交易工具、安全的存储解决方案和可靠的执行，可以充满信心地交易加密货币。',
    'UNDERSTANDING_CRTYPTOCURRENCIES' => '了解加密货币：概述',
    'UNDERSTANDING_CRTYPTOCURRENCIES_DESC_1' => '加密货币是利用加密技术进行安全交易并控制新单位创建的数字或虚拟货币。 以下是有关加密货币的关键点：',
    'UNDERSTANDING_CRTYPTOCURRENCIES_DESC_2' => '去中心化：加密货币在去中心化网络上运行，例如区块链技术，这消除了对银行等中介机构的需求。 这种去中心化提供了交易的透明度、安全性和控制力。',
    'UNDERSTANDING_CRTYPTOCURRENCIES_DESC_3' => '流行的加密货币：比特币（BTC）是第一个也是最著名的加密货币，为多种其他加密货币铺平了道路。 以太坊 (ETH)、莱特币 (LTC)、瑞波币 (XRP) 等都获得了巨大的知名度和市值。 <br/><br/>波动性：加密货币价格可能高度波动，为交易者提供机会和风险。 价格波动可能受到市场需求、监管发展、技术进步和投资者情绪等因素的影响。',
    'UNDERSTANDING_CRTYPTOCURRENCIES_DESC_4' => '使用案例：加密货币有多种用途，包括点对点交易、价值存储、投资资产以及促进区块链平台上的智能合约和去中心化应用程序 (DApp)。',
    'UNDERSTANDING_CRTYPTOCURRENCIES_DESC_5' => '保持信息灵通：了解加密货币新闻、市场趋势并了解底层技术对于成功的加密货币交易至关重要。 XS 提供资源、教育材料和实时市场数据，帮助您了解最新情况。',

    //Commodities Page
    'COMMODITIES_DESC_1' => 'XS Group is a global fintech and financial services provider with entities regulated and authorised by leading governing bodies of the most reputable financial jurisdictions around the globe.',
    'COMMODITIES_DESC_2' => 'We prioritise regulatory compliance to ensure trust, integrity, and accountability in the global financial industry. Our operations adhere to the regulations and requirements set forth by the relevant authorities in each jurisdiction where we operate.',
    'EXPERIENCE_THE_WORLD_OF_COMMODITIES' => '与全球经纪商一起体验商品交易的世界',
    'EXPERIENCE_THE_WORLD_OF_COMMODITIES_DESC' => '通过 XS 踏上激动人心的商品交易之旅。 我们的平台提供涵盖各种商品的多种差价合约工具。 大宗商品交易提供了参与全球市场的独特机会，并有可能获得丰厚的回报。',
    'COMMODITIES_TRADING_WITH_XS' => '使用 XS 进行商品交易',
    'COMMODITIES_TRADING_WITH_XS_DESC' => 'Experience the excitement of Share & Stock trading with XS.com. Take advantage of our platforms’ seamless CFD trading capabilities, offering you the opportunity to trade CFDs on thousands of global shares. Benefit from ultra-fast order execution and competitive trading conditions, ensuring that you can seize market opportunities swiftly and efficiently. Trade CFDs on shares of renowned companies in the US, UK, and EU, including some of the largest and most popular corporations.',
    'UNDERSTANDING_COMMODITIES' => '商品概览：通过 XS 探索商品交易世界',
    'UNDERSTANDING_COMMODITIES_DESC_1' => '介绍<br/>欢迎来到 XS 的商品交易世界。 大宗商品代表对我们日常生活至关重要的有形商品，例如金属、能源、农产品等。 在本概述中，我们将探讨商品交易的关键方面，包括热门商品、市场动态以及通过 XS 交易商品的优势。',
    'UNDERSTANDING_COMMODITIES_DESC_2' => '热门商品<br/> 大宗商品交易涉及广泛的资产，包括黄金、白银等贵金属，石油、天然气等能源，小麦、玉米等农产品，以及铜、铝等工业金属。 这些商品在各个行业中发挥着重要作用，并提供多样化的贸易机会。',
    'UNDERSTANDING_COMMODITIES_DESC_3' => '市场动态 <br/>大宗商品市场受到多种因素的影响，例如供需动态、地缘政治事件、天气状况、技术进步和全球经济趋势。 了解这些动态对于成功的商品交易至关重要。 XS 提供实时市场数据、分析和研究，帮助您有效驾驭商品市场',
];

?>