...
@section('opengraph')
    ...
    <meta property="og:title" content="{{ __('seo.TITLE_ACTIVITIES') }}">
    <meta property="og:description" content="{{ __('seo.DESCRIPTION_ACTIVITIES') }}">
    ...
    <meta name="twitter:title" content="{{ __('seo.TITLE_ACTIVITIES') }}">
    <meta name="twitter:description" content="{{ __('seo.DESCRIPTION_ACTIVITIES') }}">

@endsection
@section('meta_tags')
    @parent
    <title>{{ __('seo.TITLE_ACTIVITIES') }}</title>
    <meta name="description" content="{{ __('seo.DESCRIPTION_ACTIVITIES') }}">
@endsection
@section('content')

@include('layouts.banner', [
    'bannerTitle' => __('events.ACTIVITIES'),
    'bannerDescription' => __('events.ACTIVITIES_PAGE_SUBTITLE'),
    'bannerImage' => asset('/img/events-banners/activities.svg'),
    'mobileBanner' => asset('/img/events-banners/activities-mobile.webp'),
])
    <style>
    ...
    {{-- <div class="container py-5">
        <div class="row">
            <div class="col-12 mb-4">
                <h1 class="text-navy text-center text-uppercase col-4 m-auto">{{ __('presence.ACTIVITIES') }}</h1>
            </div>
            <div class="col-12">
                <img src="{{ asset('/img/activities.webp') }}" class="py-5 img-fluid w-100" alt="XSTrades.com activities">
            </div>
        </div>
    </div> --}}
            ...
            <div class="row mb-5">
                ...
                @foreach ($upcoming as $i => $expo)
                    @if (session()->get('locale') == 'ar')
                        ...
                        @else
                            <div class="col-md-3 my-5 onClickOpenPopup {{ $i % 3 != 0 ? 'offset-md-1' : '' }}">
                    @endif
        ...
        <div class="container mb-5">
            <div class="row ">
                ...
                @foreach ($prev as $i => $expo)
                    @php
                        $path = public_path('/img/prev-activities/' . $expo['dir']);
                        $images = [''];
                        if (File::exists($path)) {
                    ...
                    <div class="slick slick-fade">
                        ...
                        @foreach ($images as $img)
                            <img src="{{ File::exists($path) ? asset('/img/prev-activities/' . $expo['dir'] . '/' . $img->getRelativePathName()) : asset('/img/Savethedate.png') }}"
