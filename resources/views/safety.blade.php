...
@section('meta_tags')
    @parent
    <title>{{ __('seo.TITLE_SAFETY') }}</title>
    <meta name="description" content="{{ __('seo.DESCRIPTION_SAFETY') }}">
@endsection
@section('content')
    @php
        $sections = [
            [
                ...
                'button_url' => route('insurance', ['lang' => App::getLocale()]),
            ],
            [
                'icon' => 'img/safety/mulitple-regulations.svg',
                'button_url' => route('regulations', ['lang' => App::getLocale()]),
            ],
            [
                'icon' => 'img/safety/security-standards.svg',
                'button_url' => route('security', ['lang' => App::getLocale()]),
            ],
            [
    ...
    @include('layouts.banner', [
        'bannerTitle' => __('safety.PAGE_BANNER_TITLE'),
        'bannerDescription' => __('safety.PAGE_BANNER_SUBTITLE'),
        'bannerImage' => asset('/img/company-section/safety.svg'),
        'mobileBanner' => asset('/img/company-section/safety-mobile.webp'),
    ])
    ...
    {{-- <div class="w-100 position-relative mb-5">
        <div class="position-absolute w-100 h-100 top-0 start-0 d-flex align-items-center justify-content-center">
            <h1 class="text-white text-uppercase fw-700 fw-banner">{{ __('safety.PAGE_BANNER_TITLE') }}</h1>
        </div>
        <img id="markets-banner" src="{{ asset('/img/safety/safety_banner.png') }}" alt="" class="img-fluid w-100">
    </div> --}}
