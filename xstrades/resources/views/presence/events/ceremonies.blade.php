...
@section('meta_tags')
    @parent
    <title>{{ __('seo.TITLE_CEREMONIES') }}</title>
    <meta name="description" content="{{ __('seo.DESCRIPTION_CEREMONIES') }}">
@endsection
@section('content')

@include('layouts.banner', [
    'bannerTitle' => __('events.CEREMONIES'),
    'bannerDescription' => __('events.CEREMONIES_PAGE_SUBTITLE'),
    'bannerImage' => asset('/img/events-banners/ceremonies.svg'),
    'mobileBanner' => asset('/img/events-banners/ceremonies-mobile.webp'),
])
    <style>
    ...
    {{-- <div class="container py-5">
        <div class="row">
            <div class="col-12 mb-4">
                <h1 class="text-navy text-center text-uppercase col-4 m-auto">{{ __('presence.CEREMONIES') }}</h1>
            </div>
            <div class="col-12">
                <img src="{{ asset('/img/awards-banner.webp') }}" class="w-100 img-fluid"
                ...
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-6 position-relative cust-full-w">
                            ...
                            <img src="{{ asset('/img/Savethedate.png') }}" alt="{{ $expo['title'] }}"
            ...
            <div class="row ">
                <div class="col-12 my-5">
                    ...
                    <button onclick="toggleShowMore($(this) ,'.more-previous')"
                        class="text-secondary btn text-lg d-flex align-items-center px-0" style="font-size: 40px">
                        {{ __('events.PREVIOUS_CEREMONIES') }}
                    </button>
                </div>
                @foreach ($prev as $i => $expo)
                    @php
                        $path = public_path('/img/prev-ceremonies/' . $expo['dir']);
                        $images = [''];
                        if (File::exists($path)) {
                    ...
                    <div class="slick slick-fade">
                        ...
                        @foreach ($images as $img)
                            <img src="{{ File::exists($path) ? asset('/img/prev-ceremonies/' . $expo['dir'] . '/' . $img->getRelativePathName()) : asset('/img/Savethedate.png') }}"
