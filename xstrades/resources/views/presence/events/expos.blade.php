...
@section('opengraph')
    ...
    <meta property="og:title" content="{{ __('seo.TITLE_EXPOS') }}">
    <meta property="og:description" content="{{ __('seo.DESCRIPTION_EXPOS') }}">
    ...
    <meta name="twitter:title" content="{{ __('seo.TITLE_EXPOS') }}">
    <meta name="twitter:description" content="{{ __('seo.DESCRIPTION_EXPOS') }}">
@endsection
@section('meta_tags')
    @parent
    <title>{{ __('seo.TITLE_EXPOS') }}</title>
    <meta name="description" content="{{ __('seo.DESCRIPTION_EXPOS') }}">
@endsection
@section('content')
    @include('layouts.banner', [
        'bannerTitle' => __('events.EXPOS'),
        'bannerDescription' => __('events.EXPOS_PAGE_SUBTITLE'),
        'bannerImage' => asset('/img/events-banners/expos.svg'),
        'mobileBanner' => asset('/img/events-banners/expos-mobile.webp'),
    ])

    {{-- <div class="container py-5">
        <div class="row">
            <div class="col-12 mb-4">
                <h1 class="text-navy text-center text-uppercase col-4 m-auto">{{ __('presence.EXPOS') }}</h1>
            </div>
            <div class="col-12">
                <img src="{{ asset('/img/expos-banner.webp') }}" class="py-5 img-fluid w-100" alt="XSTrades.com at the expos">
            </div>
        </div>
    </div> --}}
    {{-- upcoming expos  --}}
    ...
    <div class="container">
        <div class="row mb-5">
            <div class="col-12 my-5">
                <div class="d-flex align-items-center">
                    <button onclick="toggleShowMore($(this) ,'.more-upcoming')"
                        ...
                        <h2>{{ __('expos.UPCOMING_EXPOS') }}</h2>
                        {{-- @if (count($upcoming_expos) > 3)
            ...
            @foreach ($upcoming as $i => $expo)
                <div
                    ...
                    <img src="{{ asset('/expo-img/expos/upcoming/' . $expo['dir']) }}" alt="{{ $expo['title'] }}"
                        class="img-fluid">
                    ...
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-6 position-relative cust-full-w ">
                                <div class="position-absolute top-0 start-0 m-4  cust-pt">
                                    <button class="btn rounded-circle bg-white text-primary close-btn"
                                        onclick="closePopup($(this))">x</button>
                                </div>

                                <img src="{{ asset('/expo-img/expos/upcoming/' . $expo['dir']) }}"
                                    alt="{{ $expo['title'] }}" class="img-fluid">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {{-- @if (count($upcoming_expos) > 3)
            <div class="more-upcoming row " style="display: none;">
                @foreach ($upcoming_expos as $i => $expo)
                    @if ($i >= 3)
                        <div class="col-md-3 onClickOpenPopup {{ $i % 3 != 0 ? 'offset-md-1' : '' }}">
                            <img src="{{ asset('/img/Savethedate.png') }}" alt="" class="img-fluid">
                            <h5 class="text-dark-grey fw-700 mt-3">{{ $expo['title'] }}</h5>
                            <p class="date mb-2">{{ $expo['date'] }}</p>
                            <p>{{ $expo['country'] }}</p>
                        </div>
                            ...
                            <div class="container">
                                <div class="row justify-content-center">
                                    <div class="col-6 position-relative">
                                        ...
                                        <img src="{{ asset('/img/Savethedate.png') }}" alt=""
    ...
    <div class="container mb-5">
        <div class="row">
            <div class="col-12 mb-5">
                ...
                <button onclick="toggleShowMore($(this) ,'.more-previous')"
                    class="text-secondary btn text-lg d-flex align-items-center px-0" style="font-size: 40px">
                    <h2>{{ __('expos.PREVIOUS_EXPOS') }}</h2>
                    {{-- @if (count($prev_expos) > 3)
                        <i class="fa-solid fa-chevron-down text-secondary ms-2" style="font-size: 40px"></i>
                    @endif --}}
                </button>
            </div>
            @foreach ($prev as $i => $expo)
                @php
                    $path = public_path('/img/prev-expos/' . $expo['dir']);
                    $images = [''];
                    if (File::exists($path)) {
                ...
                <div
                    ...
                    <div class="slick slick-fade">
                        ...
                        @foreach ($images as $img)
                            <img src="{{ File::exists($path) ? asset('/img/prev-expos/' . $expo['dir'] . '/' . $img->getRelativePathName()) : asset('/img/Savethedate.png') }}"
                    ...
                    <div class="container">
                        ...
                        <div class="row justify-content-center ">
                            ...
                            <div class="col-6 position-relative cust-full-w">
                                ...
                                <div class="position-absolute top-0 start-0 m-4 cust-pt" style="z-index: 100">
                                    <button class="btn rounded-circle bg-white text-primary close-btn"
                                        onclick="closePopup($(this))">x</button>
                                </div>
        ...
        {{-- @if (count($prev_expos) > 3)
            <div class="more-previous row " style="display: none;">
                @foreach ($prev_expos as $i => $expo)
                    @php

                        $path = public_path('/img/prev-expos/'. $expo['dir']);
                        $images = [asset('/img/Savethedate.png')];
                        if (File::exists($path)) {
                            $images = File::allFiles($path);
                        }
                    @endphp
